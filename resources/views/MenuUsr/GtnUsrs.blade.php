...
<h1 class="text-center">Gestión de usuarios</h1> <br />
...
<table class="table table-striped table-info">
 ...
 @foreach ($usrs as $items)

 @if (!$items->is_Admin==1 AND $items->role == 'user')
 	<tr>
      <td>{{ $items->name }}</td>
      <td>{{ $items->role }}</td>
...
<br /><h3>Acciones para ejecutar</h3><br />
  ...
  <div>
    <a href="{{ route('GtnAdmin.create')}}" class="btn btn-primary">Agregar Usuario</a>
  </div>
